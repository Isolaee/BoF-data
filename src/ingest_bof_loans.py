"""
Ingest Bank of Finland loan observations from the BoF FSA Open Data API (v4)
into a Delta table. Uses MERGE to ensure idempotency — safe to re-run.

Dedup key: (dataset_id, series_name, period)

v4 response structure:
  { currentPage, totalPages, pageSize, totalCount,
    items: [ { name, observations: [ {period, periodCode, value} ] } ] }
"""

import argparse
import requests
from datetime import datetime, timezone

from pyspark.sql import SparkSession
from pyspark.sql.types import (
    StructType, StructField, StringType, DoubleType, TimestampType
)
from delta.tables import DeltaTable

BASE_URL = "https://api.boffsaopendata.fi/v4"
PAGE_SIZE = 500  # number of series per request


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--dataset_id", default="MFI_PUBL")
    p.add_argument("--start_period", default="")
    p.add_argument("--end_period", default="")
    p.add_argument("--catalog", default="")
    p.add_argument("--schema", default="bof")
    return p.parse_args()


def fetch_observations(dataset_id: str, start_period: str, end_period: str) -> list[dict]:
    """Page through all series in the dataset and flatten to one row per observation."""
    params: dict = {"pageSize": PAGE_SIZE, "pageNumber": 1}
    if start_period:
        params["startPeriod"] = start_period
    if end_period:
        params["endPeriod"] = end_period

    records = []
    while True:
        resp = requests.get(
            f"{BASE_URL}/Observations/{dataset_id}",
            params=params,
            timeout=120,
        )
        resp.raise_for_status()
        data = resp.json()

        for series in data.get("items", []):
            series_name = series.get("name", "")
            for obs in series.get("observations", []):
                records.append({
                    "series_name":  series_name,
                    "period":       obs.get("period", ""),
                    "period_code":  obs.get("periodCode", ""),
                    "value":        obs.get("value"),
                })

        current_page = data.get("currentPage", 1)
        total_pages  = data.get("totalPages", 1)
        print(f"  page {current_page}/{total_pages} — {len(records):,} rows so far")

        if current_page >= total_pages:
            break
        params["pageNumber"] = current_page + 1

    return records


SCHEMA = StructType([
    StructField("dataset_id",   StringType(),    nullable=False),
    StructField("series_name",  StringType(),    nullable=False),
    StructField("period",       StringType(),    nullable=False),
    StructField("period_code",  StringType(),    nullable=True),
    StructField("value",        DoubleType(),    nullable=True),
    StructField("ingested_at",  TimestampType(), nullable=False),
])

MERGE_CONDITION = (
    "target.dataset_id  = source.dataset_id  AND "
    "target.series_name = source.series_name AND "
    "target.period      = source.period"
)


def to_rows(dataset_id: str, observations: list[dict], ingested_at) -> list[dict]:
    return [
        {
            "dataset_id":  dataset_id,
            "series_name": obs["series_name"],
            "period":      obs["period"],
            "period_code": obs["period_code"],
            "value":       obs["value"],
            "ingested_at": ingested_at,
        }
        for obs in observations
    ]


def ensure_table(spark: SparkSession, full_table: str):
    spark.sql(f"""
        CREATE TABLE IF NOT EXISTS {full_table} (
            dataset_id   STRING    NOT NULL,
            series_name  STRING    NOT NULL,
            period       STRING    NOT NULL,
            period_code  STRING,
            value        DOUBLE,
            ingested_at  TIMESTAMP NOT NULL
        )
        USING DELTA
        PARTITIONED BY (dataset_id)
        TBLPROPERTIES ('delta.enableChangeDataFeed' = 'true')
    """)


def resolve_catalog(spark: SparkSession, catalog_arg: str) -> str:
    if catalog_arg:
        return catalog_arg
    _readonly = {"system", "__databricks_internal", "spark_catalog", "samples"}
    all_catalogs = [r.catalog for r in spark.sql("SHOW CATALOGS").collect()]
    available = [c for c in all_catalogs if c not in _readonly]
    print(f"All catalogs: {all_catalogs} → writable candidates: {available}")
    if not available:
        raise RuntimeError("No writable catalogs found. Pass --catalog explicitly.")
    return available[0]


def main():
    args = parse_args()
    spark = SparkSession.builder.getOrCreate()

    catalog    = resolve_catalog(spark, args.catalog)
    full_table = f"{catalog}.{args.schema}.loan_observations"
    print(f"Target table: {full_table}")

    spark.sql(f"CREATE SCHEMA IF NOT EXISTS {catalog}.{args.schema}")
    ensure_table(spark, full_table)

    print(f"Fetching: dataset={args.dataset_id} "
          f"start={args.start_period or 'all'} end={args.end_period or 'latest'}")

    raw = fetch_observations(args.dataset_id, args.start_period, args.end_period)
    print(f"Fetched {len(raw):,} observations across all series")

    if not raw:
        print("No observations returned — nothing to write.")
        return

    ingested_at = datetime.now(timezone.utc)
    rows = to_rows(args.dataset_id, raw, ingested_at)
    df   = spark.createDataFrame(rows, schema=SCHEMA)

    DeltaTable.forName(spark, full_table) \
        .alias("target") \
        .merge(df.alias("source"), MERGE_CONDITION) \
        .whenNotMatchedInsert(values={
            "target.dataset_id":  "source.dataset_id",
            "target.series_name": "source.series_name",
            "target.period":      "source.period",
            "target.period_code": "source.period_code",
            "target.value":       "source.value",
            "target.ingested_at": "source.ingested_at",
        }) \
        .execute()

    total = spark.sql(
        f"SELECT COUNT(*) FROM {full_table} WHERE dataset_id = '{args.dataset_id}'"
    ).collect()[0][0]
    print(f"Done. Total rows in table for {args.dataset_id}: {total:,}")


if __name__ == "__main__":
    main()
